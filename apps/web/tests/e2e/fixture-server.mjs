...
const pages = [
  { id: "element-scan", kind: "element_scan", section: "elements", title: "The pieces of your Dota pattern", body: "Seventeen observable Elements make the report legible.", content: { scanning_body: "Reconstructing the signals we found. The strongest three get a closer look next.", ready_body: "Seventeen observable Elements make the report legible. The strongest three get a closer look next." }, evidence_keys: elementKeys, options: [] },
...
const report = {
  ...
  versions: { eligibility: "summary-eligibility-1.0.0", sessions: "sessions-1.0.0", features: "dna-features-4.0.0", behavior_model: "behavior-model-4.0.0", element_registry: "free-elements-4.0.0", pattern_registry: "free-patterns-4.0.0", pattern_ranking: "free-pattern-ranking-4.1.0", hero_taxonomy: "fixture-taxonomy", hero_portfolio: "hero-portfolio-1.1.0", hero_mirror: "hero-mirror-1.1.0", story: "free-story-4.1.0", copy: "free-dna-copy-4.1.0", model: "fixture-model", template: "free-dna-template-4.0.0", share_renderer: "share-svg-4.1.0", analysis_version_fingerprint: "fixture-fingerprint" },
  ...
  hero_portfolio: {
    ...
    version: "hero-portfolio-1.1.0"
  },
  story: { version: "free-story-4.1.0", ordered_pages: pages.map((item) => item.id) },
  pages,
  shares: { final: { display_name: "Fixture player", strongest_elements: elements.slice(0, 3).map((item) => ({ key: item.key, label: item.label, zone: item.zone })), strongest_patterns: patterns.slice(0, 3).map((item) => ({ key: item.key, label: item.label })), hero_portfolio: { common_thread: "Mobility", exception_hero: "Anti-Mage", pool_direction: "New heroes. Same taste." }, hero_mirror: { hero_id: 1, hero_name: "Anti-Mage" } }, privacy_defaults: { show_name: true, show_avatar: true, show_raw_id: false } },
