...
This is the **live legacy product**: Dota Report Card / Free DNA, deprecated but running in
production. This contract applies to work inside `legacy/`, and to any change elsewhere in the
repository that could affect the live product (see the root [`/AGENTS.md`](../AGENTS.md) for
which shared paths that includes).

**Tracker work must not use this file, and must not read, cite, or copy from anything under
`legacy/`.** Tracker product meaning and system behaviour live under `docs/tracker/**`; see
the root [`/AGENTS.md`](../AGENTS.md).

This file MUST be read before making changes inside `legacy/`, or changes elsewhere with the
potential to affect the live legacy product.
...
The production system is:
    ...
    Vercel / Next.js frontend (legacy/apps/web)
            ↓
    Railway / FastAPI API
            ├── PostgreSQL
            ├── Redis
...
- services/api/app/ shared modules (`core`, `storage`, `opendota`, `stratz`,
  `providers`, `ingestion`, `identity`) — these also serve the tracker;
- infra/;
- legacy/infra/runtime-artifacts/;
- migrations/; or
