...
from report_card.player_analysis_v6 import analyze_free_dna_v6
from report_card.player_analysis_v6.baselines import BaselineResolver
from report_card.player_analysis_v6.constants import (
    BASELINE_VERSION,
    BOOTSTRAP_VERSION,
    CLAIM_VERSION,
    DIAGNOSTICS_VERSION,
...
from report_card.player_analysis_v6.thresholds import MetricThreshold
        ...
        "diagnostic_questions": [
            {
                ...
                "context": _plain_json(item.context),
                "primary_hypothesis": _plain_json(item.primary_hypothesis),
                "secondary_hypothesis": _plain_json(item.secondary_hypothesis) if item.secondary_hypothesis is not None else None,
                ...
                "options": [_plain_json(option) for option in item.options],
                "observed": _plain_json(item.observed),
                "available": item.available,
                "skippable": item.skippable,
                "question_spec": _plain_json(item.question_spec),
