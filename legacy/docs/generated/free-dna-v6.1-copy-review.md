...
- Family / branch: `pool_shape` / `lifecycle`
- Rollout: `shadow_only`
- Denominator: `matches`; minimum `30` opportunities and `12` sessions
- Claim: A hero lifecycle candidate is available only for protected shadow evaluation.
- Evidence label: left-truncation-aware lifecycle
- Interpretation: First observed in the window is not the same as discovered.
- Alternatives: unobserved draft and match context; taxonomy or coverage uncertainty
- Interaction / fallback: `hero_lifecycle` / `text_evidence`
- Recommendation / verification: `none` / `none`

## `identity_eras`

- Family / branch: `pool_shape` / `eras`
- Rollout: `shadow_only`
- Denominator: `matches`; minimum `30` opportunities and `12` sessions
- Claim: An identity-era candidate is available only for protected shadow evaluation.
- Evidence label: session-block era candidate
- Interpretation: A stable year with no qualifying chapter is a valid result.
- Alternatives: unobserved draft and match context; taxonomy or coverage uncertainty
- Interaction / fallback: `identity_eras` / `text_evidence`
- Recommendation / verification: `none` / `none`

## `behavioral_loop`

- Family / branch: `session_drift` / `motif`
- Rollout: `shadow_only`
- Denominator: `occurrences`; minimum `30` opportunities and `12` sessions
- Claim: A sequence candidate is available only for protected shadow evaluation.
- Evidence label: non-overlapping motif occurrences
- Interpretation: No loop is public without discovery and independent verification support.
- Alternatives: unobserved draft and match context; taxonomy or coverage uncertainty
- Interaction / fallback: `behavioral_loop` / `text_evidence`
- Recommendation / verification: `none` / `none`
