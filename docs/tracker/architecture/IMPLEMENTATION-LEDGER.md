...
### Retained-source role correction and mobile Edit Role checkpoint

- The finalizer's analytical calculation is now reusable without side effects. A profile-scoped role correction appends a user assertion with expected revision, checks retained source and supported original methodology versions, atomically remeasures the corrected match and later same-mode old/new-role dependency closure, and rebuilds the affected current baseline and PB pointers. Other roles and the other bucket retain their active pointers. Equal PB values keep the earliest source; no delivered event is retracted or resent. READY matches outside progression can change role from retained evidence with target-only rebuild and no history writes. Missing lineage fails closed. No provider read occurs.
- Match Detail exposes role revision and source-backed correction availability; authenticated `POST /matches/{match_ref}/role` enforces active-profile visibility, idempotency and stale-revision rejection. Focused PostgreSQL mobile/correction/pipeline plus legacy contracts: **23 passed, 0 failed, 0 skipped**. Ruff/mypy pass on affected application modules. An integration test projects Stage 1 from stored summary evidence, runs fixture-only replay success or terminal absence, executes private workers, and reads READY from mobile with measured versus N/A values. It does not yet prove the complete sync/summary acquisition or two-account dedup path.
- Remaining correction work: recompute Profile claim aggregates and record coherent Change receipts where displayed state changes; complete methodology/parameter rebuilds, full historical recovery/entitlement interplay, and the goal-wide E2E matrix. This is a partial Phase E/G checkpoint, not phase closure. No provider calls, push, merge or deployment.

## Continuation by Opus (handoff from Sol) — 2026-09-25

Handoff state verified before changes: branch `codex/tracker-backend-foundation` at `70ac88f` plus Sol's uncommitted insight-history comparator and subscription-view work. Local PostgreSQL 16 (55432) and Redis 7.2.16 (56379) were running. Baseline with Sol's working tree: **277 passed, 1 failed** (Sol's new retained-history test imported helpers from the wrong module and claimed a bootstrap finalization on P0). Fixed the test, preserved and committed Sol's work. Asana is skipped for this run by owner instruction; this ledger is the progress record.

### App Store JWS verification and retained insight history (`afcebd7`)

- `tracker/app_store.py`: ES256 JWS verifier behind `AppStoreVerifier`. It requires a three-certificate `x5c` chain ending in an operator-pinned root, validity at signing time, parent signatures, CA basic constraints on the intermediate and the Apple marker OIDs; nested notification transactions and renewal info are verified independently and must match. No OCSP/revocation check (network I/O) — recorded as an operational follow-up. Built only when `TRACKER_APP_STORE_ROOT_CERT_PATH` and `TRACKER_APP_STORE_BUNDLE_ID` are configured.
- Mobile `POST /subscription/transactions` (idempotency key, 503 when unconfigured) and `GET /subscription`. Server-to-server `POST /store/app-store/notifications` is a separate app with no OpenAPI document.
- Finalization now supplies strictly prior, entitled, READY retained comparators to insight cards (Sol's `load_retained_history`).
- Evidence: locally generated root/intermediate/leaf chain; 10 App Store tests (pinned root, other root, bundle, tamper, missing marker, expired leaf, HS256 substitution, nested notification, mobile submit/replay/foreign transaction, refund webhook, fail-closed without verifier). **No production Apple verification is claimed.**

### Shared rebuild engine, context wiring and Profile checkpoints (`428dbc9`)

- **Defect found and fixed:** finalization wrote current baseline/PB rows from `priors + current` only, so a match admitted later at an older chronology position (bootstrap refill, backfill, recovery) could replace a newer rolling window or take the PB from a better later match. Current indexes are now recomputed from all entitled READY history after the link is READY (a second ordering defect — recompute before the READY flip — was caught by the new regression test). Regression: `test_late_older_match_cannot_take_newer_pb_or_rolling_window`.
- `tracker/rebuild.py`: one provider-free replay path (`replay_closure`) for entitlement scope changes (`SCOPE_REBUILD`, P3), methodology/version bumps and context parameter-set changes (`METHODOLOGY_REBUILD`, P3, closure = each bucket from its earliest stale chronology point). Publication is one transaction under user → profile locks; analysis identity `(profile, match, analysis_version, inputs_digest)` makes a second run reuse every row. Scope rebuilds bump the revision and write one in-app `SCOPE_CHANGED` event; no NEW_PB/READY events.
- `context.evaluate` now persists `h` independently of the baseline gates (annex §8 step 6) so later matches have window priors; finalization stores `h`/`E` levels and `parameter_set_version` per observation and builds the draft from internal positions and replay lanes. A user correction changes the effective role only; the viewer position is implied by core roles, and support keeps a 4/5 assignment; disagreements fail closed (lane UNAVAILABLE). Parameter sets are read from `tracker_parameter_sets` kind `CONTEXT_POPULATION`, status `APPROVED` only; none exists, so adjustment degrades to zero and performance is `NOT_READY` in production.
- `tracker/profile.py` + migration `0014_tracker_profile_states`: per-bucket projection published at coherent checkpoints (live READY, settled bootstrap mode, role correction, scope/methodology rebuild); importing buckets keep their prior checkpoint. Derives only SSOT-fixed values (counts, "so far" fallback line, role shares/tiers with ±3 pp hysteresis at n≥30, most-played heroes). Claims, hero tags and Right now stay `CALIBRATION_PENDING` (see owner decisions). Change events are written only for confirmed identity changes, so none occur until claim parameters exist.
- Evidence: `tests/tracker/test_rebuild.py` (5): late-admission regression; Free→Pro→Free scope rebuild with pre-link history, revision 1→2, identical Free analysis row reused, zero provider calls, no new events; parameter-set change rebuilds Standard only (Turbo pointer unchanged), second run no-op; Profile fallback identity and withheld claims; importing bucket keeps its checkpoint. Full tracker + migration units + contract: **304 passed, 0 failed, 0 skipped**.

### Pro backfill, data-access recovery and mobile inventory (`4164b88`)

- `tracker/backfill.py`: `PRO_BACKFILL` (pre-link) and `ACCESS_RECOVERY` (original link date → now) profile-scoped P3 scans that queue the existing historical batches (origins HISTORICAL / RECOVERY) only for unheld supported matches. Pro depth ceiling is `TRACKER_PRO_HISTORY_DAYS`; unset means no backfill (owner decision open). Pro activation waits (`WAITING_FOR_HISTORY`) while historical work is unsettled; one backfill per profile generation is reused on resubscription (no refetch).
- **Defect fixed:** a live match waited (`WAITING_FOR_PRIOR_MATCH`) on any earlier unresolved link, so an import could stall fresh work. Live work now waits only on earlier live links (bootstrap keeps its per-mode gate).
