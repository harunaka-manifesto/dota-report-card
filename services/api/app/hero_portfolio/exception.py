...
def compute_hero_exception(
    ...
    options = _options(distances, winner, taxonomy)
    confidence = min(
        1.0,
    ...
    if winner_distance < 0.32 or margin < 0.06:
        return HeroExceptionResult(
            ...
            exception_traits=(),
            options=tuple((*options, ChoiceOption(key="no_clear_exception", label="No clear exception"))),
...
def _no_clear(
    ...
    options = tuple(
        ...
        for item in sorted(candidates, key=lambda value: (-value.matches, value.hero_id))[:3]
    )
    return HeroExceptionResult(
        status="unavailable",
        hero_id=None,
        ...
        pool_traits=(),
        exception_traits=(),
        options=options + (ChoiceOption(key="no_clear_exception", label="No clear exception"),),
        correct_option_key="no_clear_exception",
        distance=None,
        margin=None,
