...
The gap is readiness, not provider coupling: `obs_placed` and `sen_placed` are replay-class, so the classifier's summary-class evidence profile is materially weaker than its replay-class one. It will still produce a role (lane role and economy ranking are summary-class), but with lower confidence and more Support/Offlane ambiguity.
